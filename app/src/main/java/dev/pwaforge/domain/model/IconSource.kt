...
sealed class IconSource {
    ...
    /** Simple Icons SVG — slug for CDN + background hex color (e.g. "#3F51B5") */
    data class SvgIcon(val slug: String, val background: String) : IconSource()

    fun toJson(): String = when (this) {
        is Path -> JSONObject().put("type", "path").put("path", path).toString()
        is SvgIcon -> JSONObject().put("type", "svg").put("slug", slug).put("background", background).toString()
    }
        ...
        fun fromJson(json: String?): IconSource? {
            ...
            return runCatching {
                ...
                when (obj.getString("type")) {
                    ...
                    "svg" -> SvgIcon(obj.getString("slug"), obj.getString("background"))
                    else -> null
                }
            }.getOrNull()
        }
